...
class Baligam implements ICoupon {
	const API_LOGIN_URL     = '###';
	...
	public function convert( $data ): Coupon {
		...
		$coupon->set_title( $this->regex_title_price( $data['price'], "{$data['newsletter_title']} {$data['title']}" ) );
	...
	private function regex_title_price( $price, $title ): string {
		return str_replace( '[MIN_PRICE]', $price, $title );
	}
}
